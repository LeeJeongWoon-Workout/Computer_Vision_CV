...
'''-----------------------------------------------------------------------------------data'''
!mkdir /content/data
!wget https://public.roboflow.com/ds/Gkbg3iHmlH?key=HL93dUSEP3
!unzip -qq '/content/Gkbg3iHmlH?key=HL93dUSEP3' -d '/content/data'
...
'''-----------------------------------------------------------------------------------------------------------------vudei detectuib'''
import numpy as np
def get_detected_img(model, img_array,  score_threshold=0.9, is_print=True):
  # 인자로 들어온 image_array를 복사. 
  draw_img = img_array.copy()
  bbox_color=[(0,0,255),(255,0,0)]
  text_color=(0, 0, 255)

  # model과 image array를 입력 인자로 inference detection 수행하고 결과를 results로 받음. 
  # results는 80개의 2차원 array(shape=(오브젝트갯수, 5))를 가지는 list. 
  results = inference_detector(model, img_array)
  labels_to_names_seq= ('mask','no-mask') 
...
def do_detected_video(model, input_path, output_path, score_threshold=0.6, do_print=True):
...
model.cfg=cfg
do_detected_video(model,'/content/data/mask_Trim.mp4','/content/data/mask_Trim_out.mp4')
