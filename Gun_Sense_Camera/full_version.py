...
!cd /content/data/train && unzip export.zip 
!cd /content/data/test && unzip export.zip 
!cd /content/data/valid && unzip export.zip 

from mmdet.datasets.builder import DATASETS
from mmdet.datasets.coco import CocoDataset

@DATASETS.register_module(force=True)
class RSPDataset(CocoDataset):
  CLASSES =("pistol",)
...
cfg.dataset_type = 'RSPDataset'
cfg.data_root = '/content/data/'

# train, val, test dataset에 대한 type, data_root, ann_file, img_prefix 환경 파라미터 수정. 
cfg.data.train.type = 'RSPDataset'
cfg.data.train.data_root = '/content/data/'
cfg.data.train.ann_file = 'annotations/pistol_annotations.coco.json'
cfg.data.train.img_prefix = 'train'

cfg.data.val.type = 'RSPDataset'
cfg.data.val.data_root = '/content/data/'
cfg.data.val.ann_file = 'annotations/pistol_annotations.coco.json'
cfg.data.val.img_prefix = 'test'

cfg.data.test.type = 'RSPDataset'
cfg.data.test.data_root = '/content/data/'
cfg.data.test.ann_file = 'annotations/pistol_annotations.coco.json'
cfg.data.test.img_prefix = 'valid'
...
import numpy as np
import cv2
def get_detected_img(model, img_array,  score_threshold=0.95, is_print=True):
  ...
  for result_ind, result in enumerate(results):
    ...
    for i in range(len(result_filtered)):
      ...
      cv2.rectangle(draw_img, (left, top), (right, bottom), color=bbox_color[result_ind], thickness=3)
...
def do_detected_video(model, input_path, output_path, score_threshold=0.95, do_print=True):
...
model.cfg=cfg
do_detected_video(model,'/content/data/gun.mp4','/content/data/gun_out.mp4')
